...
export class TemplatePayloadService {
  ...
  decodeBase64ToYaml(encoded: string): string {
    try {
      ...
      throw new Error(
        `Failed to decode base64 payload to yaml: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  decodeBase64ToObject(encoded: string): ComposeJson {
    try {
      ...
      throw new Error(
        `Invalid encoded compose payload: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  encodeObjectToBase64(payload: unknown): string {
    try {
      ...
      throw new Error(
        `Failed to encode compose payload: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
