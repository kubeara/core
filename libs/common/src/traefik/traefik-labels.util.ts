...
export function buildTraefikLabels(options: BuildTraefikLabelsOptions): Record<string, string> {
    try {
        ...
        labels[`traefik.http.services.${httpRouterName}.loadbalancer.server.port`] = String(internalPort);

        if (enableHttps) {
            const httpsRouterName = `${safeRouterId}-https`;
            labels[`traefik.http.routers.${httpsRouterName}.rule`] = `Host(\`${host}\`)`;
            labels[`traefik.http.routers.${httpsRouterName}.entrypoints`] = 'https';
            labels[`traefik.http.routers.${httpsRouterName}.service`] = httpsRouterName;
            labels[`traefik.http.services.${httpsRouterName}.loadbalancer.server.port`] = String(internalPort);
            ...
            if (forceHttps) {
                ...
                labels[`traefik.http.routers.${httpRouterName}.middlewares`] = 'redirect-to-https';
            }
        }

        return labels;
    } catch (error) {
        throw new Error(`Failed to build Traefik labels: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export function sanitizeRouterId(value: string): string {
    try {
        ...
        throw new Error(`Failed to sanitize router id: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export function discoverTraefikRoutes(
    ...
    try {
        ...
        for (const declaration of extractUrlFqdnDeclarations(composeYaml)) {
            ...
            const parsedEnvironmentVariable = parseServiceEnvironmentVariable(declaration);
            ...
            const host = env[fqdnKey] ?? env[`SERVICE_FQDN_${parsedEnvironmentVariable.preservedName}`];
            if (!host) {
                continue;
            }

            const internalPort = parsedEnvironmentVariable.port ? Number(parsedEnvironmentVariable.port) : 80;
        ...
        throw new Error(`Failed to discover Traefik routes: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export function applyTraefikRoutingToCompose(
    ...
    try {
        ...
        throw new Error(`Failed to apply Traefik routing to compose: ${error instanceof Error ? error.message : String(error)}`);
    }
}
