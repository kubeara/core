...
export class ComposeParserService {
  ...
  extractVariables(compose: string) {
    try {
      ...
      throw new Error(
        `Failed to extract compose variables: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  resolveFromCompose(options: ResolveComposeEnvOptions): ResolvedComposeEnv {
    try {
      ...
      throw new Error(
        `Failed to resolve compose values: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  resolveAndValidateFromCompose(
    ...
    try {
      ...
      throw new Error(
        `Failed to resolve and validate compose values: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  inferRequiredVariables(
    ...
    try {
      ...
      throw new Error(
        `Failed to infer required compose variables: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  findMissingVariables(
    ...
    try {
      ...
      throw new Error(
        `Failed to find missing compose variables: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  listPortVariables(compose: string): string[] {
    try {
      ...
      throw new Error(
        `Failed to list compose port variables: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  findUnknownPortKeys(
    ...
    try {
      ...
      throw new Error(
        `Failed to find unknown port keys: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
