...
export class SshCommandExecutorService {
    ...
    async executeCommand(clientOrId: Client | string, command: string, timeoutMs = SSH_DEFAULTS.COMMAND_TIMEOUT): Promise<ExecuteResult> {
        const start = Date.now();

        const execOnClient = (client: Client) => new Promise<ExecuteResult>((resolve, reject) => {
            let stdout = '';
            let stderr = '';
        ...
        if (typeof clientOrId === 'string') {
            ...
            if (!client) throw new SshCommandError('No active SSH connection for server');
        } else {
            client = clientOrId;
        }
        ...
        try {
            ...
            this.logger.log(`Executed command on ssh; len_stdout=${res.stdout.length}`);
            return res;
        } catch (err) {
    ...
    async executeCommands(clientOrId: Client | string, commands: string[], timeoutMs?: number) {
