...
export class SshConnectionManager {
    ...
    async connect(options: SshConnectionOptions): Promise<Client> {
        ...
        if (options.authType === 'PASSWORD' || options.encryptedPassword) {
            const pwd = options.encryptedPassword ? this.encryptionService.decrypt(options.encryptedPassword) : undefined;
            (connectConfig as any).password = pwd;
        }
        ...
        if (options.authType === 'PRIVATE_KEY') {
            const key =
                ...
                (options.encryptedPrivateKey
                    ? await this.encryptionService.decrypt(options.encryptedPrivateKey)
                    : undefined);
            ...
            (connectConfig as any).privateKey = key;
        }
        ...
        return new Promise<Client>((resolve, reject) => {
            ...
            client.on('ready', () => {
                this.logger.log(`SSH ready for server=${options.serverId} host=${options.host}`);
            ...
            client.on('error', (err: Error) => {
                this.logger.warn(`SSH error server=${options.serverId} host=${options.host} msg=${String(err.message)}`);
                if (!resolved) {
            ...
            try {
                ...
                const safeConfig = { ...connectConfig, privateKey: connectConfig.privateKey ? '[REDACTED]' : undefined };
    ...
    async reconnect(options: SshConnectionOptions): Promise<Client> {
        await this.disconnect(options.serverId);
