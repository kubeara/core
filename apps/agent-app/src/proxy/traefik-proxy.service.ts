...
export class TraefikProxyService {
    ...
    isEnabled(): boolean {
        try {
            ...
            throw new Error(`Failed to evaluate TRAEFIK_ENABLED flag: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    isHttpsEnabled(): boolean {
        try {
            ...
            throw new Error(`Failed to evaluate TRAEFIK_HTTPS flag: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    isForceHttps(): boolean {
        try {
            ...
            throw new Error(`Failed to evaluate TRAEFIK_FORCE_HTTPS flag: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    getProxyComposePath(): string {
        try {
            ...
            throw new Error(`Failed to resolve Traefik compose path: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async ensureRunning(): Promise<void> {
        try {
            ...
            const dockerArguments = ['compose', '-f', composePath, '-p', this.projectName, 'up', '-d'];

            const executionResult = await this.exec('docker', dockerArguments, process.cwd());
            ...
            throw new Error(`Failed to ensure Traefik proxy is running: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
        ...
        try {
            return new Promise((resolve) => {
                ...
                childProcess.on('close', (code) => {
                    resolve({ exitCode: code ?? 1, stdout: standardOutput, stderr: standardError });
                });
            });
        } catch (error) {
            return Promise.reject(new Error(`Failed to execute command "${command}": ${error instanceof Error ? error.message : String(error)}`));
        }
    }
}
