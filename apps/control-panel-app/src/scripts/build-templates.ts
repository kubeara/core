...
for (const file of files) {
    ...
    let configData: Record<string, unknown> | undefined = undefined;
    ...
    if (stat.isDirectory()) {
        ...
        if (fs.existsSync(configPath)) {
            try {
                ...
                console.warn(`Failed to parse template.config.json for ${slug}: ${String(e)}`);
            }
        }
    } else {
    ...
    if (configData) {
        if (configData.env_schema) serviceTemplate.env_schema = configData.env_schema;
        if (configData.port_schema) serviceTemplate.port_schema = configData.port_schema;
    }
