...
export class DeploymentGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    afterInit(): void {
        try {
            ...
            this.logger.error(`Failed during websocket gateway initialization logging: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    handleConnection(client: Socket): void {
        try {
            ...
            this.logger.error(`Failed to handle agent connection: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    handleDisconnect(client: Socket): void {
        try {
            ...
            this.logger.error(`Failed to handle agent disconnect: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async handleDeploymentStatus(client: Socket, payload: DeploymentStatusPayload): Promise<void> {
        try {
            ...
            this.logger.error(`Failed to process deployment status event: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    handleDeploymentLog(client: Socket, payload: DeploymentLogPayload): void {
        try {
            ...
            this.logger.error(`Failed to process deployment log event: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    getConnectedAgents(): string[] {
        try {
            ...
            this.logger.error(`Failed to get connected agents: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
    ...
    getConnectedAgentsCount(): number {
        try {
            ...
            this.logger.error(`Failed to get connected agent count: ${error instanceof Error ? error.message : String(error)}`);
            return 0;
        }
    }
    ...
    getPrimaryAgentPublicIp(): string | null {
        try {
            ...
            this.logger.error(`Failed to get primary agent public IP: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }
    }
}