import { discoverTemplateComposeFiles } from "./template-validation/discover-template-compose-files.util";
import {
...
describe("service template compose files", () => {
  ...
  describe.each(
    discoveredTemplates.map((template) => [template.slug, template]),
  )("template %s", (_slug, template) => {
    ...
    it("builds a database-ready template record from source files", () => {
      ...
      expect(
        Object.keys(parsed.services as Record<string, unknown>).length,
      ).toBeGreaterThan(0);
