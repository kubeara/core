...
function validateComposeStructure(
...
function validateHeaderMetadata(
  ...
  const headerLines = composeYaml
    .split("\n")
    .filter((line) => line.startsWith("#"));

  const hasPortHint = headerLines.some((line) => /^#\s*port\s*:/i.test(line));
  const hasDocumentation = headerLines.some((line) =>
    /^#\s*documentation\s*:/i.test(line),
  );
  ...
  if (!hasDocumentation) {
    issues.push({
      ...
      message:
        "Template header must include a `# documentation: <url>` comment",
    });
  }
}
 ...
 */
export function listComposeServiceNames(
  parsed: Record<string, unknown>,
): string[] {
  const services = parsed.services;
...
function serviceExposesHostEndpoint(service: Record<string, unknown>): boolean {
  ...
  return (
    listComposePortVariables(portText).length > 0 ||
    extractComposeVariables(portText).length > 0
  );
}
...
function validateServicePortMappings(
  ...
  const servicePortVars = portVars.filter((name) =>
    name.startsWith("SERVICE_PORT_"),
  );
  ...
  if (servicePortVars.length === 0) {
    issues.push({
      path: basePath,
      message:
        "Published port mappings must use SERVICE_PORT_* variables (compose-only templates)",
    });
  }
...
function schemaPortsDeclaredInCompose(
  context: TemplateComposeFileContext,
): boolean {
  ...
  return (
    ...
    schemaPortKeys.every(
      (portKey) =>
        context.composeYaml.includes(`\${${portKey}`) ||
        context.composeYaml.includes(`$${portKey}`),
    )
  );
}
...
function validateHealthcheck(
  ...
  if (!block.test) {
    issues.push({
      path: `${path}.test`,
      message: "Healthcheck must define a test command",
    });
  }
...
function validateResourceLimits(
  ...
  if (!deploy || typeof deploy !== "object" || Array.isArray(deploy)) {
    issues.push({
      path,
      message: "Service must define deploy.resources.limits (cpu and memory)",
    });
    return;
  }
  ...
  if (!limits || typeof limits !== "object" || Array.isArray(limits)) {
    issues.push({
      path: `${path}.limits`,
      message: "Service must define deploy.resources.limits",
    });
    return;
  }
...
function validateLoggingLimits(
  ...
  if (!logging || typeof logging !== "object" || Array.isArray(logging)) {
    issues.push({
      path,
      message:
        "Service must define logging.options.max-size and logging.options.max-file",
    });
    return;
  }
  ...
  if (maxSize === undefined || maxSize === null || maxSize === "") {
    issues.push({
      ...
      message: "logging.options.max-size is required",
    });
  } else {
    const sizeBytes = parseMemoryToBytes(formatComposeValue(maxSize));
    if (sizeBytes === null) {
      issues.push({
        path: `${path}.options.max-size`,
        message: `Invalid logging max-size value: ${formatComposeValue(maxSize)}`,
      });
    } else if (sizeBytes > policy.maxLogFileSizeBytes) {
      issues.push({
        path: `${path}.options.max-size`,
        message: `logging max-size ${formatComposeValue(maxSize)} exceeds platform maximum of ${formatBytes(policy.maxLogFileSizeBytes)}`,
      });
    }
  }
  ...
  if (maxFile === undefined || maxFile === null || maxFile === "") {
    issues.push({
      ...
      message: "logging.options.max-file is required",
    });
  } else {
    ...
    if (!Number.isFinite(count) || count <= 0) {
      issues.push({
        path: `${path}.options.max-file`,
        message: `Invalid logging max-file value: ${formatComposeValue(maxFile)}`,
      });
...
function validateEnvironmentRules(
  ...
  const composeVars = extractComposeVariables(context.composeYaml).map(
    (entry) => entry.name,
  );
  const invalidBareDollar = composeVars.filter(
  ...
  const missingWithoutContext = findMissingComposeVariables(
    context.composeYaml,
    resolved,
  ).filter(
    (name) =>
      !name.startsWith("SERVICE_URL_") && !name.startsWith("SERVICE_FQDN_"),
  );
...
function hasTraefikRouting(service: Record<string, unknown>): boolean {
  ...
  return Object.keys(labels).some((key) => key.startsWith("traefik."));
}
