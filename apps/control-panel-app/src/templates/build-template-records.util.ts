...
function encodeComposeToBase64(composeJson: Record<string, unknown>): string {
    try {
        ...
        throw new Error(`Failed to encode compose payload: ${toErrorMessage(error)}`);
    }
}
 ...
 */
function parseComposeYaml(yamlContent: string, slug: string, sourcePath: string): Record<string, unknown> {
    try {
 ...
 */
function parseTemplateConfig(configPath: string, slug: string): Record<string, unknown> {
    try {
 ...
 */
export function buildServiceTemplateRecords(templatesDir: string): ServiceTemplateRecord[] {
    try {
        ...
        for (const file of files) {
            ...
            if (stat.isDirectory()) {
                ...
                try {
                    ...
                    throw new Error(`Failed to read template "${slug}": ${toErrorMessage(error)}`);
                }
            ...
            if (configData?.env_schema) {
                record.env_schema = configData.env_schema as Record<string, unknown>;
            }

            if (configData?.port_schema) {
                record.port_schema = configData.port_schema as Record<string, unknown>;
            }
        ...
        throw new Error(`Failed to build service template records: ${toErrorMessage(error)}`);
    }
}
 ...
 */
export function getDefaultTemplatesDir(rootDir: string = process.cwd()): string {
