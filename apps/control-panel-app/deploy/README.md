...
Set `CONTROL_PANEL_URL` on the control panel (e.g. `http://host.docker.internal:3410` when the agent runs in Docker and the panel on the host). Local agent files default to `~/.kubeara/agent` (override with `KUBEARA_AGENT_LOCAL_DIR`).
...
The console service mounts `console-nginx.conf` (ships with this directory; also regenerated by `install.sh`), which serves the SPA and proxies `/api` and `/socket.io` to the control panel. The SPA and API therefore share one origin, so auth cookies work no matter which host you browse from (`localhost`, `127.0.0.1`, LAN IP). The upstream port inside the config must match `PORT` in `.env.control-panel` (default 3410) — edit it there if you change `PORT`.
...
- Control panel API: http://localhost:3410
- Console SPA: http://localhost:8080

The console proxies the API through its own origin by default (`VITE_API_URL=/api`), so no extra CORS or
cookie configuration is needed. If you point the console at a remote panel, set `VITE_API_URL` in
`.env.control-panel` to that panel's absolute URL.
...
#       CONTROL_PANEL_URL (e.g. http://host.docker.internal:3410)
...
| `PORT` | Control panel port (default 3410) |
| `CONSOLE_PORT` | Console SPA host port (default 8080) |
| `VITE_API_URL` | Console API base URL. Default `/api` (same-origin via console nginx proxy); set an absolute URL to override |
