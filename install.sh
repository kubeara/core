...
#   KUBEARA_IMAGE_PULL       always|missing|never (default: always). Use "missing"
#                            to keep locally built images and pull only absent ones.
...
Environment variables:
  ...
  KUBEARA_PUBLIC_URL         e.g. http://203.0.113.10:3410 or https://panel.example.com
...
default_local_api_url() {
  echo "http://127.0.0.1:${PORT:-3410}"
}

# Console SPA calls the API through its own origin (nginx proxy /api + /socket.io
# to the control panel), so VITE_API_URL is relative and auth cookies work on any host.
...
services:
  ...
  control-panel-app:
    ...
    environment:
      ...
      CONTROL_PANEL_URL: ${CONTROL_PANEL_URL:-http://localhost:3410}
  ...
  console:
    ...
    environment:
      # Relative URL: the SPA and API share one origin via the nginx proxy
      # (console-nginx.conf), so auth cookies work on any host.
...
# Console nginx config: serves the SPA and proxies /api + /socket.io to the
# control panel so the SPA and API share one origin (avoids cross-site SameSite
# cookie blocking). Must run after create_env_file so PORT is final.
write_console_nginx_conf() {
  local dest="${KUBEARA_INSTALL_DIR}/console-nginx.conf"
  local port
  port="$(grep -E '^PORT=' "${KUBEARA_INSTALL_DIR}/${ENV_FILE}" | cut -d= -f2- || echo 3410)"

  cat >"${dest}" <<NGINX_EOF
# Console (SPA) served by nginx, with /api and /socket.io proxied to the control
# panel so the SPA and API share one origin. Same-origin avoids cross-site
# SameSite cookie blocking and CORS entirely, regardless of the host the user
# browses from (localhost, 127.0.0.1, LAN IP, hostname). Generated by install.sh.
...
server {
  ...
  set \$control_panel_upstream http://kubeara-control-panel:${port};
  ...
  location /api/ {
    proxy_pass \$control_panel_upstream;
  ...
  location ~* \\.(?:css|js|mjs|map|ico|png|jpg|jpeg|gif|svg|webp|woff|woff2)$ {
    expires 7d;
...
ensure_core_env_config() {
  ...
  # The control panel now defaults to a rare host port (3410) so it does not
  # collide with common local dev backends on 3000. Migrate legacy defaults.
  current="$(grep -E '^[[:space:]]*PORT=' "${env_path}" 2>/dev/null | head -n1 | cut -d= -f2- || true)"
  if [[ "${current}" == "3000" || "${current}" == "3001" ]]; then
    set_env_var "PORT" "3410" "${env_path}"
    # Refresh auto-generated URL/CORS entries that embed the legacy port.
    sed -E -i 's|(http://[^:]+):300[01]|\1:3410|g' "${env_path}"
    warn "Control panel port migrated from ${current} to 3410 to avoid collisions with common dev ports. If you customized CONTROL_PANEL_URL/VITE_API_URL/CORS, review them."
  fi

  # The console now talks to the API through its own origin (nginx proxy), so a
  # same-host absolute VITE_API_URL is replaced with the relative /api default.
  # This fixes cross-site SameSite cookie blocking when browsing via
  # 127.0.0.1 / LAN IP instead of localhost.
  current="$(grep -E '^[[:space:]]*VITE_API_URL=' "${env_path}" 2>/dev/null | head -n1 | cut -d= -f2- || true)"
  if [[ "${current}" =~ ^http://(localhost|127\.0\.0\.1|0\.0\.0\.0):[0-9]+(/api)?$ ]]; then
    set_env_var "VITE_API_URL" "/api" "${env_path}"
    warn "Console VITE_API_URL switched to /api (same-origin proxy). Set VITE_API_URL to an absolute URL to override."
  fi

  port="$(grep -E '^[[:space:]]*PORT=' "${env_path}" 2>/dev/null | head -n1 | cut -d= -f2- || echo 3410)"
  ...
  # Self-hosted installs use reverse SSH tunnels so remote agents can reach the
  # control panel (agents cannot dial the control panel directly).
  if ! grep -qE '^[[:space:]]*SELF_HOSTED=' "${env_path}"; then
    set_env_var "SELF_HOSTED" "${SELF_HOSTED:-true}" "${env_path}"
  fi
...
main() {
  ...
  case "${KUBEARA_IMAGE_PULL:-always}" in
    always)
      info "Pulling images…"
      compose pull
      ;;
    missing)
      info "KUBEARA_IMAGE_PULL=missing — skipping explicit pull; compose will pull only images that are absent locally."
      ;;
    never)
      info "KUBEARA_IMAGE_PULL=never — skipping pull (startup fails if images are missing)."
      ;;
    *)
      warn "Unknown KUBEARA_IMAGE_PULL='${KUBEARA_IMAGE_PULL}' (expected always|missing|never); treating as always."
      compose pull
      ;;
  esac
